...
def example_converter(example, dtype=torch.float32,
                             device=None) -> dict:
    device = device or torch.device("cuda:0")
    example_torch = {}
    float_names = [
    ...
    # print('before create label TIME:'+str(now.to_sec()))
    ...
    # now = rospy.get_rostime()
    # print('after create label TIME:'+str(now.to_sec()))
    return example_torch
...
def _predict_pc(net,example,class_names):
    now = rospy.get_rostime()
    print('before detection TIME:'+str(now.to_sec()))
    predictions_dicts = net(example)
    now = rospy.get_rostime()
    print('after detection TIME:'+str(now.to_sec()))
    for i,preds_dict in enumerate(predictions_dicts):
        if preds_dict["box3d_lidar"] is not None:
            ...
            print(box_preds_lidar)

            return box_preds_lidar,label_preds,scores
...
defo_point = np.zeros((1,3,4),dtype = np.float32) 
        



def pointp_based_detection(model_dir,
             net,input_cfg,model_cfg,train_cfg,class_names,voxel_generator,target_assigner,
             points = defo_point,
             result_path=None,
             predict_test=False,
             ckpt_path=None,
    ...
    eval_dataset = input_reader_builder.build_evaluation(
        points,
        input_cfg,
        model_cfg,
        False,
    ...
    # print('create dataset TIME:'+str(now.to_sec()))
    ...
    # print('create dataloader TIME:'+str(now.to_sec()))
    ...
    for example in iter(eval_dataloader):
        ...
        now = rospy.get_rostime()
        print('before convert TIME:'+str(now.to_sec()))
        example = example_converter(example, float_dtype)
        now = rospy.get_rostime()
        print('after convert TIME:'+str(now.to_sec()))


        if(_predict_pc(net, example, class_names)):
            return _predict_pc(net, example, class_names)
        else:
...
if __name__ == '__main__':
         fire.Fire()
        #  #evaluate('/home/these/second.pytorch/second/configs/pointpillars/ped_cycle/xyres_16.proto',
        #     '/home/these/second.pytorch/path/for_ped/model_dir',
        #      None,
        #      True,
        #      None,
        #      None,
