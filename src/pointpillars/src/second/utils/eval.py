...
def clean_data(gt_anno, dt_anno, current_class, difficulty):
    ...
    for i in range(num_gt):
        bbox = gt_anno["bbox"][i]
        ####
        dimension = gt_anno["dimensions"][i]
        location = gt_anno["location"][i]
        #######
        ...
        if (gt_name == current_cls_name):
         ...
         # #シミュレーションでやるときのみ使う #######################################################################
        #print(dimension)
        if (dimension[2] == 0.7600005 and dimension[1] < 1.831 and dimension[1] > 1.829):
            valid_class = 0
            #print(valid_class)

        if (dimension[2] > 0.509 and dimension[2] < 0.511 and dimension[1] > 1.699 and dimension[1] < 1.701):
            valid_class = 0
            #print(valid_class)
        ...
        if (np.sqrt(location[2]*location[2] + location[0]*location[0])  >= 2.5):
            valid_class = 0
            #print(valid_class)
        ...
        # #シミュレーションでやるときのみ使う ##########################################################
        # if (dimension[2] == 0.7 and dimension[1] == 0.7 and dimension[0] == 1.77):
        #     ignore = True
        #     print(ignore)
    ...
    for i in range(num_dt):
        ...
        #     #シミュレーションでやるときはここ変える -> append(1) -> append(0)
        #     ignored_dt.append(1))
            #ignored_dt.append(1)
        if (np.sqrt(dt_anno["location"][i,2]*dt_anno["location"][i,2] +dt_anno["location"][i,0]*dt_anno["location"][i,0]) >= 2.5):
            ignored_dt.append(1)
